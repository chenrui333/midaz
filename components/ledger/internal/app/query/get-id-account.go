...
func (uc *UseCase) GetAccountByID(ctx context.Context, organizationID, ledgerID, portfolioID, id string) (*a.Account, error) {
	...
	account, err := uc.AccountRepository.Find(ctx, uuid.MustParse(organizationID), uuid.MustParse(ledgerID), uuid.MustParse(portfolioID), uuid.MustParse(id))
	if err != nil {
	...
	if account != nil {
		metadata, err := uc.MetadataRepository.FindByEntity(ctx, reflect.TypeOf(a.Account{}).Name(), id)
		if err != nil {
